  ...
  return (
    <div className="relative">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-40" onMouseLeave={() => setHoverIdx(null)}>
        {gridLines.map((g) => (
          <line key={g} x1={padL} x2={width - padR} y1={padT + plotH * (1 - g)} y2={padT + plotH * (1 - g)} stroke="var(--chart-grid)" strokeWidth={1} />
        ))}
        <path d={areaPath} fill={color} opacity={0.1} />
        <path d={linePath} fill="none" stroke={color} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
        {hoverIdx !== null && (
          <line x1={xFor(hoverIdx)} x2={xFor(hoverIdx)} y1={padT} y2={padT + plotH} stroke="var(--chart-crosshair)" strokeWidth={1} />
        )}
        {data.map((d, i) => (
          <circle
            key={i}
      ...
      {hoverIdx !== null && (
        <div
          className="absolute theme-tooltip text-[10px] rounded-lg px-2 py-1 pointer-events-none -translate-x-1/2 -translate-y-full shadow-lg"
        ...
        >
          ...
          <div className="text-[var(--text-muted)]">{new Date(data[hoverIdx].date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</div>
        </div>
      )}
