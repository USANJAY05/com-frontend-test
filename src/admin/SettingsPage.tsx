...
export default function SettingsPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-16 text-slate-400"><Loader2 className="h-5 w-5 animate-spin mr-2" /> Loading features…</div>;
  ...
  const renderFlags = (items: FeatureFlag[]) => (
    <div className="divide-y divide-slate-100 dark:divide-[var(--border)]">
        ...
        <div key={flag.key} className="flex items-center justify-between gap-4 py-3.5">
          <div className="min-w-0 pr-4">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-slate-500 dark:bg-[var(--bg-subtle)] dark:text-[var(--text-muted)]">{flag.key}</span>
            </div>
            <div className="mt-0.5 text-xs text-slate-400 dark:text-[var(--text-muted)]">{flag.description}</div>
          </div>
      ...
      {items.length === 0 && <div className="py-8 text-center text-xs text-slate-400">No flags in this category.</div>}
    </div>
  );
    ...
    <div className="grid grid-cols-12 gap-4">
      ...
      <Widget colSpan={12} title="Feature groups" subtitle="Reusable organization/team bundles. Globally disabled product features cannot be selected." padding="md">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-[var(--text-primary)]"><Users2 className="h-4 w-4" /> Groups</div>
          <button type="button" onClick={() => setGroupDraft({ key: '', label: '', description: '', featureKeys: [] })} className="inline-flex items-center gap-1.5 rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white"><Plus className="h-3.5 w-3.5" /> New group</button>
        </div>
        <div className="space-y-2">
          {groups.map(group => (
            <div key={group.key} className="flex items-center justify-between rounded-xl border border-slate-200 dark:border-[var(--border)] px-3 py-3">
              <div><div className="text-sm font-medium text-slate-700 dark:text-[var(--text-primary)]">{group.label}</div><div className="text-xs text-slate-400">{group.description || 'No description'} · {group.featureKeys.length} features</div></div>
        ...
        {groupDraft && (
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 dark:bg-[var(--bg-subtle)] p-4">
            <div className="grid gap-3 md:grid-cols-2">
              <input value={groupDraft.key} disabled={!!groupDraft.system} onChange={e => setGroupDraft({ ...groupDraft, key: e.target.value })} placeholder="Group key" className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs" />
              <input value={groupDraft.label} onChange={e => setGroupDraft({ ...groupDraft, label: e.target.value })} placeholder="Group name" className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs" />
            </div>
            <input value={groupDraft.description} onChange={e => setGroupDraft({ ...groupDraft, description: e.target.value })} placeholder="Description" className="mt-3 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs" />
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {appFeatures.map(feature => {
                const selected = groupDraft.featureKeys.includes(feature.key);
                return <button type="button" key={feature.key} onClick={() => setGroupDraft({ ...groupDraft, featureKeys: selected ? groupDraft.featureKeys.filter(k => k !== feature.key) : [...groupDraft.featureKeys, feature.key] })} disabled={!feature.enabled} className={`rounded-lg border px-3 py-2 text-left text-xs ${selected ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600'} ${!feature.enabled ? 'opacity-40' : ''}`}><div className="font-semibold">{feature.label}</div><div className="mt-0.5 text-[10px] text-slate-400">{feature.enabled ? 'Available' : 'Globally disabled'}</div></button>;
              })}
            </div>
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button type="button" disabled={savingGroup || !groupDraft.key || !groupDraft.label} onClick={async () => { setSavingGroup(true); try { await apiFetch('/api/platform/feature-groups', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(groupDraft) }); setGroupDraft(null); await load(); } finally { setSavingGroup(false); } }} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50"><Save className="h-3.5 w-3.5" /> Save group</button>
            </div>
          </div>
        )}
      </Widget>
